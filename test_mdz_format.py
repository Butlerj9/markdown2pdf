#!/usr/bin/env python3
"""
MDZ Format Test Script
--------------------
This script tests the MDZ format implementation.

File: test_mdz_format.py
"""
...
def test_mdz_bundle():
    ...
    try:
        ...
        if not os.path.exists(extracted_img_path):
            logger.error(f"Extracted image not found: {extracted_img_path}")
            return False
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="MDZ Format Test Script")
    ...
    if not (args.bundle or args.renderer or args.all):
        args.all = True
