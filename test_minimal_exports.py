#!/usr/bin/env python3
"""
Minimal test script for export verification with a small subset of settings
to ensure tests complete within timeout limits
"""
...
def main():
    ...
    export_timeout = int(os.environ.get("TEST_TIMEOUT", "30"))
    ...
    # Sample markdown content for testing (minimal version)
...
"""
    ...
    # PDF engines to test (just one for speed)
    pdf_engines = ["xelatex"]
    ...
    for toc in toc_settings:
        for page in page_settings:
            for font in font_settings:
                for format_setting in format_settings:
                    for export_format in export_formats:
                        ...
                        # Define output file
                        settings_desc = f"toc{'on' if toc['include'] else 'off'}_depth{toc['depth']}"
                        ...
                        if export_format == "pdf":
                            # For PDF, test with default engine
                            print(f"  Exporting PDF with default engine (timeout: {export_timeout}s)...")
                            result, timed_out, exception = timeout_handler(export_timeout, window.export_to_pdf, output_file)
                            ...
                            if result:
                                ...
                                if timed_out:
                                    ...
                                    issue = f"PDF export error with default engine: {str(exception)}"
                            ...
                            result, timed_out, exception = timeout_handler(export_timeout, window.export_to_html, output_file)
                            if result:
                                ...
                                if timed_out:
                                    ...
                                    issue = f"HTML export error: {str(exception)}"
                            ...
                            result, timed_out, exception = timeout_handler(export_timeout, window.export_to_epub, output_file)
                            if result:
                                ...
                                if timed_out:
                                    ...
                                    issue = f"EPUB export error: {str(exception)}"
                            ...
                            result, timed_out, exception = timeout_handler(export_timeout, window.export_to_docx, output_file)
                            if result:
                                ...
                                if timed_out:
                                    ...
                                    issue = f"DOCX export error: {str(exception)}"
